...
# 6.5. PREPARACIÓN DEL EXCEL DEL MODELO Y EJECUCIÓN DE LA PROGRAMACIÓN
# _______________________________________________________________________
...
def preparar_disponibilidad_recursos_modelo() -> pd.DataFrame:
    filas = []
    for fecha_dia in rango_fechas(st.session_state.horizonte_inicio, st.session_state.horizonte_fin):
        wd = int(pd.Timestamp(fecha_dia).weekday())
        for recurso_modelo, por_dia in DISPONIBILIDAD_RECURSOS_MODELO.items():
            for h_ini, h_fin in por_dia.get(wd, []):
                filas.append({
    ...
    return pd.DataFrame(filas, columns=["recurso", "fecha", "inicio", "fin"])
...
def ejecutar_modelos_desde_excel(ruta_excel: str) -> Dict[str, Any]:
    ...
    modelo_milp_app.TIME_LIMIT_SOLVER = TIEMPO_MAXIMO_BUSQUEDA_SEG
    ...
    try:
        ...
        pass
    modelo_milp_app.DURACIONES_ESTANDAR["salud_ocupacional"] = 20
...
modulo = st.radio(
    ...
    options=["Disponibilidad recursos", "Registro de pacientes", "Disponibilidad pacientes", "Programar EMO", "Conjuntos guardados"],
    horizontal=True,
